...
?>
			...
			<div style="background:url(<?php echo $space->getProfileBannerImage()->getUrl(); ?>) center center; background-size:cover">


				<div class="p-sm bg-white-overlay text-center b-b">

					<a href="" class="w-64 inline-block"><img src="img/u0.png" class="img-circle img-responsive"></a>
 ...
 -				<?php echo \humhub\modules\space\widgets\Image::widget(['space' => $space, 'width' => 64]); ?>
			...
			<?php endif; ?>
					...
					<div class="m-v-sm h3"><span class="text-black"><?php echo Html::encode($space->name); ?></span></div>
					<em class="text-muted small"><?php echo Html::encode($space->description); ?></em>
				</div>
			</div>
			<div class="row no-margin p-v-sm b-b">
				<div style="text-align: center;" class="col-xs-4">
					<span class="h3 block font-bold"><?php echo $postCount; ?></span>
					<em class="text-xs"><?php echo Yii::t('SpaceModule.widgets_views_profileHeader', 'Posts'); ?></em>
				</div>
				<div style="text-align: center;" class="col-xs-4">
				<a href="<?= $space->createUrl('/space/membership/members-list'); ?>" data-target="#globalModal" class="waves">
					<span class="h3 block font-bold"><?php echo $space->getMemberships()->count(); ?></span>
					<em class="text-xs"><?php echo Yii::t('SpaceModule.widgets_views_profileHeader', 'Members'); ?></em>
				</a>
				</div>
				<div style="text-align: center;" class="col-xs-4">
				<a href="<?= $space->createUrl('/space/space/follower-list'); ?>" data-target="#globalModal" class="waves">
					<span class="h3 block font-bold"><?php echo $space->getFollowerCount(); ?></span>
					<em class="text-xs"><?php echo Yii::t('SpaceModule.widgets_views_profileHeader', 'Followers'); ?></em>
				</a>
				</div>
			</div>
					<div  class="row no-margin p-v-sm b-b">
	...
	<div class="panel-profile-header">
		...
		<div class="image-upload-container" style="width: 100%; height: 100%; overflow:hidden;">
			<!-- profile image output-->
			<img class="img-profile-header-background" id="space-banner-image" src="<?php echo $space->getProfileBannerImage()->getUrl(); ?>" width="100%" style="width: 100%;">

			<!-- check if the current user is the profile owner and can change the images -->
			<?php if ($space->isAdmin()) { ?>
				<form class="fileupload" id="bannerfileupload" action="" method="POST" enctype="multipart/form-data" style="position: absolute; top: 0; left: 0; opacity: 0; width: 100%; height: 100%;">
